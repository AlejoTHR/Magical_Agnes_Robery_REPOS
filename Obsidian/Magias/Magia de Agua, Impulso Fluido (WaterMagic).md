...
* **Restricción de Impulso Aéreo:**  Si Agnes está en el suelo (`plymov.isGrounded()`), puede realizar el *dash* siempre que el *cooldown* esté listo.
  * Si se encuentra en el aire, solo se le permite un único desplazamiento por salto. Esto se controla mediante la bandera `DashUsed`, la cual cambia a `true` inmediatamente al iniciar la rutina.
...
2. **Direccionamiento Dinámico:** El script consulta la dirección de entrada actual del jugador a través de `plymov.ReturnDirection()`. Si la lectura es neutra ($0$), la física calcula la orientación del impulso basándose en la escala local del sprite en el eje x para proyectarlo hacia el frente:
   * Si $\text{localScale.x} > 0 \rightarrow \text{Dirección} = 1.0$ (Derecha).
   * Si $\text{localScale.x} < 0 \rightarrow \text{Dirección} = -1.0$ (Izquierda).
3. **Bucle de Fuerza Bloqueante:** Durante el tiempo que dura el desplazamiento, se fuerza frame a frame en el ciclo físico (`WaitForFixedUpdate`) una velocidad constante anulando por completo el eje y. Esto asegura que Agnes flote horizontalmente sin verse afectada por la gravedad o por las lecturas normales del script base de movimiento:
   $$V_x = \text{dashPower} \times \text{Dirección}$$
   $$V_y = 0.0$$

#### Salida del Estado de Impulso
Una vez completado el tiempo de la animación de desplazamiento (`elapsed >= dashDuration`), el sistema detiene en seco al personaje fijando su velocidad a cero para devolverle el control de forma limpia. Se reactiva el componente de fuego (`fire.enabled = true`) y se limpian las banderas de estado elemental del controlador de animaciones (`usingWaterMagic = false`, `usingFireMagic = false`), regresando a la locomoción neutra.